...
const uiManifest = JSON.parse(readFileSync(new URL('../../packages/ui/package.json', import.meta.url), 'utf8'));
...
try {
  for (const pkg of packages) {
    ...
    if (!tarball) {
      throw new Error(`Could not find packed tarball for ${pkg.manifest.name}@${pkg.manifest.version}`);
    }
